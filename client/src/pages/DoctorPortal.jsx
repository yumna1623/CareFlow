...
const Icon = ({ name, className = "w-5 h-5" }) => {
    ...
    const icons = {
        // Dashboard Icons
        'calendar': 'M6 2a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V4a2 2 0 00-2-2H6zm0 2h12v4H6V4zm0 6h12v8H6v-8z',
        ...
        'logout': 'M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3v-4a3 3 0 013-3h7',
        'hospital': 'M20 6H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V8a2 2 0 00-2-2zM4 8h16v10H4V8zm4 4v-2h2v2h2v-2h2v2h2v-2h-2V10h-2V8h-2v2h-2V8h-2v2h-2v2h2zm4 0h-2v2h2v-2z',
        'check': 'M5 13l4 4L19 7',
        'waiting': 'M10 18a8 8 0 100-16 8 8 0 000 16zM10 14V8',
        'total': 'M12 6.75a.75.75 0 01.75-.75h.75a.75.75 0 01.75.75v10.5a.75.75 0 01-.75.75h-.75a.75.75 0 01-.75-.75V6.75zM8.25 10.5a.75.75 0 01.75-.75h.75a.75.75 0 01.75.75v6.75a.75.75 0 01-.75.75h-.75a.75.75 0 01-.75-.75V10.5zM15.75 8.25a.75.75 0 01.75-.75h.75a.75.75 0 01.75.75v9a.75.75 0 01-.75.75h-.75a.75.75 0 01-.75-.75v-9z',
        'queue': 'M19 11a1 1 0 011 1v1a1 1 0 01-1 1h-1v-2h1zM5 11a1 1 0 01-1 1v1a1 1 0 011 1h1v-2H5zm14-4a1 1 0 011 1v1a1 1 0 01-1 1h-1V7h1zM5 7a1 1 0 01-1 1v1a1 1 0 011 1h1V7H5zm10 0a1 1 0 011 1v7a1 1 0 01-1 1h-4a1 1 0 01-1-1V8a1 1 0 011-1h4zM9 11h6v2H9v-2z',
            ...
            <div className="w-1/2 bg-blue-800 p-16 flex flex-col justify-between text-white">
                <div>
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <div>
                            <h1 className="text-3xl font-extrabold tracking-tight">CareFlow</h1>
                ...
                <div>
                    ...
                    <p className="text-lg opacity-80 mb-12">
                        Gain comprehensive oversight of your appointments, patient flow, and practice metrics in real-time.
                    </p>
            ...
            <div className="w-1/2 p-16 flex items-center justify-center">
                <div className="w-full max-w-md bg-white p-10 rounded-xl shadow-2xl border border-gray-100">
                    ...
                    <p className="text-gray-500 mb-8">
                        {isLogin ? "Access your professional dashboard." : "Set up your clinic account."}
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <Icon name="mail" className="absolute left-3 top-3.5 w-5 h-5 text-gray-400" />
                                <input
                                    className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 focus:border-blue-500 focus:outline-none"
                                    type="email"
...
function DoctorDashboard({ auth, setAuth }) {
    ...
    // State to manage sidebar navigation, though only one view exists now.
    ...
    const waitingList = appointments.filter(
        (a) => a.status === "scheduled" && a.appointment_id !== nowServing?.appointment_id
    );
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            ...
            <div className="w-64 bg-blue-800 shadow-xl flex flex-col justify-between p-6">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3 mb-10">
                        <Icon name="hospital" className="w-8 h-8 text-white" />
                        <h1 className="text-xl font-extrabold text-white tracking-wide">CareFlow</h1>
                    </div>
                    ...
                    <nav className="space-y-2">
                        ...
                        />
                        {/* Add more links here for other parts of the dashboard if needed */}
                    </nav>
                </div>
                ...
                <button
                    ...
                    className="flex items-center gap-3 bg-blue-700 hover:bg-blue-600 text-white px-4 py-3 rounded-lg font-medium transition w-full shadow-lg"
                >
            ...
            <div className="flex-1 p-10 overflow-y-auto">
                <h2 className="text-3xl font-bold text-gray-800 mb-8">Appointments Dashboard</h2>

                {/* Date & Hours Card (High-Contrast, Blue Theme) */}
                <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-8">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-4">
                             ...
                             <div>
                                <p className="text-gray-500 text-sm font-medium mb-1">Viewing Appointments For</p>
                                <input
                                    type="date"
                                    value={selectedDate}
                        ...
                        <div className="text-right flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-gray-500 text-sm font-medium mb-1">Clinic Hours</p>
                                <p className="text-xl font-bold text-gray-800">{doctorDetails.start_time || 'N/A'} - {doctorDetails.end_time || 'N/A'}</p>
                                <p className="text-gray-500 text-xs">Slot Duration: {doctorDetails.slot_duration || 15} min</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="col-span-2">
                        {nowServing ? (
                            <div className="bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
                                ...
                                <div className="p-8">
                                    <h2 className="text-4xl font-extrabold text-gray-800 mb-6">{nowServing.patient_name}</h2>
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <div className="text-xl text-gray-600 space-y-2">
                                                <p><span className="font-semibold text-gray-800">Queue Number:</span> {nowServing.queue_number}</p>
                                                <p><span className="font-semibold text-gray-800">Age:</span> {nowServing.patient_age || "N/A"}</p>
                                            </div>
                                        </div>
                                        <button
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                            ...
                            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                                {waitingList.length > 0 ? (
                                    waitingList.map((p) => (
                                        <div key={p.appointment_id} className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                                            <p className="font-bold text-orange-700 text-lg flex justify-between items-center">
                                                {p.slot_time} 
                                                <span className="text-sm bg-orange-200 px-2 py-0.5 rounded-full font-extrabold">Q: {p.queue_number}</span>
                                            </p>
                                            <p className="text-base text-gray-700 mt-1 font-medium">{p.patient_name}</p>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-center text-gray-400 py-4">The waiting list is empty.</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                            ...
                            <div className="space-y-2 max-h-32 overflow-y-auto pr-2">
                                {completedList.length > 0 ? (
                                    completedList.map((p) => (
                                        <div key={p.appointment_id} className="text-sm text-gray-700 pb-2 border-b border-gray-100 last:border-b-0">
                                            <span className="font-semibold text-gray-900">{p.slot_time}</span> - {p.patient_name}
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-center text-gray-400 py-2">No appointments completed yet.</p>
                                )}
                            </div>
                        </div>
                    </div>
...
const SidebarLink = ({ icon, label, isActive, onClick }) => {
    ...
    return (
        <button
            onClick={onClick}
            className={`flex items-center gap-3 w-full px-4 py-3 rounded-lg font-medium transition ${isActive ? activeClasses : inactiveClasses}`}
        >
...
const StatCard = ({ title, value, icon, color }) => {
    ...
    const classes = colorClasses[color] || 'border-gray-500 text-gray-600 bg-white';
...
export default function DoctorPortal() {
    ...
    useEffect(() => {
        ...
        if (token && !auth) {
            axios
                .get(`${API}/doctor/me`, { headers: { Authorization: `Bearer ${token}` } })
                .then((res) => {
